  ...
  def season_summary(season, team_id)
    ...
    reg_wins_percent = wins_percentage(season, "R")
      if pre_wins_percent.include?(team_id)
        p_win_percent_val = pre_wins_percent.find {|team,wins| team == team_id}
        summary[:preseason][:win_percentage] = p_win_percent_val[1].round(2)
      else summary[:preseason][:win_percentage] = 0.0
      end
      if reg_wins_percent.include?(team_id)
        r_win_percent_val = reg_wins_percent.find {|team,wins| team == team_id}
        summary[:regular_season][:win_percentage] = r_win_percent_val[1].round(2)
      else summary[:regular_season][:win_percentage] = 0.0
      end
    preseason_games = game_by_type(season, "P")
    p_away_goals_scored = away_goals_scored(preseason_games, team_id)
    p_home_goals_scored = home_goals_scored(preseason_games, team_id)
    p_goals_scored = p_away_goals_scored + p_home_goals_scored
    summary[:preseason][:goals_scored] = p_goals_scored
    p_away_goals_allowed = away_goals_allowed(preseason_games, team_id)
    p_home_goals_allowed = home_goals_allowed(preseason_games, team_id)
    p_goals_allowed = p_away_goals_allowed + p_home_goals_allowed
    summary[:preseason][:goals_against] = p_goals_allowed
    regular_season_games = game_by_type(season, "R")
    r_away_goals_scored = away_goals_scored(regular_season_games, team_id)
    r_home_goals_scored = home_goals_scored(regular_season_games, team_id)
    r_goals_scored = r_away_goals_scored + r_home_goals_scored
    summary[:regular_season][:goals_scored] = r_goals_scored
    r_away_goals_allowed = away_goals_allowed(regular_season_games, team_id)
    r_home_goals_allowed = home_goals_allowed(regular_season_games, team_id)
    r_goals_allowed = r_away_goals_allowed + r_home_goals_allowed
